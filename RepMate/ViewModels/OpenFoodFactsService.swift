...
struct OpenFoodFactsService {
    ...
    /// Scans database for product by EAN/UPC barcode. Throws if network fails, returns nil if product not found.
    static func fetchProduct(barcode: String) async throws -> ProductResult? {
        ...
        guard httpResponse.statusCode == 200 else {
            if httpResponse.statusCode == 404 {
                return nil // Product not found, which is a valid non-error state
            }
        ...
        guard let status = json["status"] as? Int, status == 1,
            ...
            return nil // Valid response, but product not found in DB
        }
